...
import { loadAtlas, DefaultMascotSequencer } from "../src/modules/mascot-sequencer/mascot-sequencer.js";
...
const sp: ScriptPackage = {
  ...
  bodySteps: [
    { text: "1. The classic for loop", code: "for (let i = 0; i < 3; i++) {\n  console.log(i);\n}", language: "javascript" },
    { text: "2. for...of — cleaner", code: "for (const n of [0, 1, 2]) {\n  console.log(n);\n}", language: "javascript" },
    { text: "3. forEach — functional", code: "[0, 1, 2].forEach((n) => console.log(n));", language: "javascript" },
  ],
  voiceoverText: "Here are three ways to loop in JavaScript. First, the classic for loop. Second, for of, which reads cleaner. Third, for each, the functional way!",
...
const voice = new DefaultVoiceSynthesizer(
  new ElevenLabsTtsClient({ apiKey: envVal("ELEVENLABS_API_KEY"), voiceId: envVal("ELEVENLABS_VOICE_ID") }),
  "./data/tmp",
);
...
const seq = new DefaultMascotSequencer(await loadAtlas("assets/mascot/coddy-atlas.json"), "./data/tmp");
